  ...
  <div class="biz-page editor-page">
    ...
    <div ref="scrollArea" class="editor-scroll"><div class="editor-content">
      ...
      <template v-else-if="step === 2">
        ...
        <section class="editor-group"><div class="editor-group-head"><h3>关联案例</h3><p>从店铺案例库选择，最多关联 6 个；也可在这里新建。</p></div><div class="case-selection"><ElSelect v-model="form.caseIds" multiple :multiple-limit="6" collapse-tags :max-collapse-tags="2" placeholder="选择店铺案例"><ElOption v-for="item in c.data.cases" :key="item.id" :label="item.title" :value="item.id" /></ElSelect><ElButton @click="openCase"><Plus :size="14" />新建案例</ElButton></div><div v-if="linkedCases.length" class="linked-cases"><span v-for="item in linkedCases" :key="item.id" :title="item.title">{{ item.title }}</span></div></section>
        <section class="editor-group"><div class="editor-group-head faq-head"><div><h3>常见问题</h3><p>按客户阅读顺序排列，最多 10 组；问题与回答需成对填写。</p></div><ElButton :disabled="form.faqs.length >= 10" @click="form.faqs.push({ question: '', answer: '' })"><Plus :size="14" />添加问题</ElButton></div><div v-if="!form.faqs.length" class="editor-empty-note">暂无常见问题。可添加客户在下单前最关心的问题。</div><div v-for="(faq,index) in form.faqs" :key="index" class="faq-card"><div class="faq-card-head"><span>问题 {{ index + 1 }}</span><div class="faq-card-actions"><button type="button" :disabled="index === 0" title="上移" @click="moveFaq(index,-1)"><ArrowUp :size="15" /></button><button type="button" :disabled="index === form.faqs.length - 1" title="下移" @click="moveFaq(index,1)"><ArrowDown :size="15" /></button><button type="button" title="删除问题" @click="form.faqs.splice(index,1)"><Trash2 :size="15" /></button></div></div><div class="editor-grid faq-fields"><label class="biz-field">问题 <ElInput v-model="faq.question" maxlength="30" show-word-limit placeholder="输入客户可能提出的问题" /><small v-if="errors[`faq-${index}-question`]" class="field-error">{{ errors[`faq-${index}-question`] }}</small></label><label class="biz-field">回答 <ElInput v-model="faq.answer" type="textarea" :rows="4" maxlength="200" show-word-limit placeholder="给出明确、简短的回答" /><small v-if="errors[`faq-${index}-answer`]" class="field-error">{{ errors[`faq-${index}-answer`] }}</small></label></div></div></section>
      </template>
...
<style scoped>
.faq-fields{grid-template-columns:1fr;gap:13px}.faq-fields :deep(.el-textarea__inner){min-height:110px!important}.regions-field :deep(.el-input__wrapper),.case-selection :deep(.el-select__wrapper){min-height:36px;max-height:44px;overflow:hidden;align-items:center}.regions-field :deep(.el-cascader__tags),.case-selection :deep(.el-select__selection){max-height:34px;overflow:hidden}.regions-field :deep(.el-tag),.case-selection :deep(.el-tag){max-width:180px}
</style>
