...
export interface Order { id: string; serviceId: string; serviceName: string; parkId: string; category: string; customer: string; amount: number; paid: number; deliverySnapshot?: string; status: OrderStatus; createdAt: string; contractFile: string; contractState: 'none' | 'waiting' | 'rejected' | 'confirmed'; deliverables: string[]; deliveryNote: string; acceptanceAt: string; completedAt: string; afterSaleEnd: string; shareRate: number; afterSaleDays: number; refunded: number; settlement: 'waiting' | 'settled' }
